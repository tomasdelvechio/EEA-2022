...
-   [**dplyr**](https://dplyr.tidyverse.org/) y [**tidyr**](https://tidyr.tidyverse.org/) para consultar, realizar transformaciones y agregaciones sobre los sets de datos
-   [**lubridate**](https://lubridate.tidyverse.org/) para trabajar con datos tipo fecha
-   [**stringr**](https://stringr.tidyverse.org/) para trabajar con datos tipo string
-   [**ggplot2**](https://ggplot2.tidyverse.org/) para realizar gráficos
...
Datos %>% 
  group_by(FECHA) %>%
  summarise(Indprom = mean(INDICE), Indlen2 = length(INDICE))
```
...
Para más detalle sobre joins, ver notebook opcional de distintos tipos de [joins](https://eea-uba.github.io/EEA-2021/clase%202/Clase%202.%20Distintos%20tipos%20de%20Join%20(OPCIONAL).nb.html).
...
`ggplot` tiene **muchos** comandos, y no tiene sentido saberlos de memoria, es siempre útil reutilizar gráficos viejos y tener a mano el [machete](data-visualization.pdf).
...
Si queremos agregar la dimensión *sexo*, podemos utilizar el comando `facet_wrap()`
...
Si lo que queremos hacer es poner el foco de atención en las diferencias por sexo, *neutralizamos* el efecto del nivel educativo, realizando la apertura por nivel educativo.
